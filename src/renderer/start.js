  ...
  (() => {
    ...
    $("#up1").click(function () {
      if (
        ...
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y12  " + 1;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y13  " + 0;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
      } else {
    ...
    $("#down1").click(function () {
      if (
        ...
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y13  " + 1;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y12  " + 0;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
      } else {
        var tanchu = "先连接服务器之后,开启自动刷新,才能修改";
    ...
    $("#on2").click(function () {
      if (
        ...
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y14  " + 1;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y15  " + 0;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
      } else {
        var tanchu = "先连接服务器之后,开启自动刷新,才能修改";
    ...
    $("#off2").click(function () {
      if (
        ...
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y15  " + 1;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
        var mycommand =
          pathToAsset1 + " -w " + myservename + " Channel1.Device1.y14  " + 0;
        console.log("执行命令", mycommand);
        cmd.get(mycommand, mycallbacktongyong);
      } else {
        var tanchu = "先连接服务器之后,开启自动刷新,才能修改";
          ...
          if (usefuloutput1.length > 0) {
            ...
            if (chazhaofuwuqi.length > 0) {
              ...
              if (document.getElementById("zdsx").checked == true) {
                console.log("执行命令", pathToAsset1 + " -d " + myservename);
    ...
    function mycallback2(err, data, stderr) {
      ...
      if (!err) {
        ...
        if (usefuloutput.length > 0) {
          ...
          $("#myfreetime").text(
            Math.round((my空闲时间次数 / allrefreshcishu) * 100 * 100) / 100 +
              "%"
          );
          $("#mybusytime").text(
            100 -
          ...
          $("#my1floortime").text(
            Math.round((my停靠1层次数 / allrefreshcishu) * 100 * 100) / 100 +
              "%"
          );
          $("#my2floortime").text(
            Math.round((my停靠2层次数 / allrefreshcishu) * 100 * 100) / 100 +
              "%"
          );
          $("#my3floortime").text(
            Math.round((my停靠3层次数 / allrefreshcishu) * 100 * 100) / 100 +
              "%"
          );
          $("#my4floortime").text(
            Math.round((my停靠4层次数 / allrefreshcishu) * 100 * 100) / 100 +
              "%"
          );
          if ($("#myx16").text() == "1") {
            my开门次数++;
